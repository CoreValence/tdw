...
#      headers and drops tdw.so + control + sql into Debian's standard
#      postgres install paths.
#   2. `runtime` is stock postgres:18-bookworm with those three files copied in,
#      so the extension is loadable via shared_preload_libraries=tdw.
...
WORKDIR /build
COPY Cargo.toml Cargo.lock tdw.control ./
COPY .cargo ./.cargo
COPY src ./src
...
# so the loader finds it next to tdw.so. aarch64 accepts the equivalent
...
RUN --mount=type=cache,target=/usr/local/cargo/registry,id=cargo-${TARGETARCH} \
    ...
    cp /usr/lib/postgresql/18/lib/tdw.so /export/lib/; \
    cp /usr/share/postgresql/18/extension/tdw.control /export/ext/; \
    cp /usr/share/postgresql/18/extension/tdw--*.sql /export/ext/; \
    if [ "$(uname -m)" = "x86_64" ]; then \
